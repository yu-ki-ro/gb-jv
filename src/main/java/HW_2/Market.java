...
public class Market implements QueueBehaviour, MarketBehaviour {
    ...
    public void acceptToMarket(Actor actor) {
        ...
        for (Actor actorInList : actors) {
            if ((actorInList instanceof Human)
                ...
                System.out.printf("Пользователь: %s уже имеет доступ на магазин%n",
                        (((Human) actor).getName()));
                isNameFound = true;
                break;
            }
        ...
        if (!isNameFound && actor instanceof Human) {
            actors.add(actor);
            System.out.printf("Пользователь: %s -> Получил доступ на магазин%n",
                    (((Human) actor).getName()));
        }
    }
    ...
    public void releaseFromMarket(List<Actor> actors) {
        for (Actor actorInList : actors) {
            if (actorInList.isTakeOrder()) {
                System.out.printf("Пользователь: %s -> получил заказ и был выгружен из магазина",
    ...
    public void update() {
        ...
        System.out.println("Состояние магазина: ");
    ...
    public void takeInQueue(Actor actor) {
        ...
        if (!isUserFound && actor instanceof Human) {
            System.out.printf("Пользователь: %s не найден в магазине%n",
                    (((Human) actor).getName()));
        }
    }
    ...
    public void takeOrders() {
        ...
        for (Actor actor : actors) {
            if ((actor instanceof Human) && actor.isMakeOrder()) {
                System.out.printf("Пользователь: %s -> Заказ был принят магазином%n",
                        (((Human) actor).getName()));
                isUserFound = true;
            }
        }
    ...
    public void releaseFromQueue() {
        for (int i = actors.size() - 1; i >= 0; i--) {
            if ((actors.get(i) instanceof Human) && actors.get(i).isTakeOrder()) {
                ...
                System.out.printf("Пользователь: %s -> Удален из очереди магазина%n",
                        (((Human) actors.get(i)).getName()));
